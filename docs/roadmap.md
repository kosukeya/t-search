...
- **R4d — completed:** [R4 synthesis](t_search_r4_synthesis.md) and [15-claim ledger](../results/t_search_r4_claim_ledger.md). Explain states, records, facts, probabilities and global histories; map overlap and remaining choices in RQM, stable facts, CPL and consistent histories. Global representation and internal record change coexist in the specified model. Close R4 as an understanding/reproduction/comparison study; a new interpretation, single-outcome account and ontological becoming remain unestablished.

Both full CI runs on R4b head `1a8f79b04bdb7a9b7f356858048e9089a9af0a91` succeeded; the PR log confirms 1405 tests and 18 subtests. R4c/R4d documentation-head CI is tracked separately in PR #22. R4b's source-hashed protocols, code and data remain fixed; latest status is recorded here and in the R4 synthesis.

R4a–R4d are complete. No R4e or R5 is selected, and closing this bounded task does not reject the broader research question. The old Stage 17 search remains frozen because it is not needed for this question. The blocked pilot and strict cyclic rejection remain historical limits. Earlier planning and closure statements describe their original checkpoints. R4a/R4b/R4c reports and source-hashed specifications are preserved; R0–R3 code, numerical results and claim ledgers remain unchanged.
...
- **R1 — completed with blockers:** [literature and implementability audit](t_search_r1_preflight.md). The distinction between a physical observable and a timed device remains essential.
- **R1b — completed, strict candidate rejected:** [finite readout-device audit](t_search_r1b_readout_design_audit.md), [code](../experiments/r1b_readout_design_audit.py), and [results](../results/r1b_readout_design_audit.json). One copying interaction per closed three-event cycle excludes the canonical input. A boundary inverse interaction repairs the coherent model but exceeds the declared budget.
- **Pilot:** `pilot_gate = blocked`; RQ2 closure and RQ3 control experiments have not started. Novelty remains unestablished.
- **Boundary supplement — completed by explicit user instruction:** [proof and comparison](t_search_boundary_supplement.md), [code](../experiments/r_boundary_supplement.py), and [results](../results/r_boundary_supplement.json). Removing the closing edge restores arbitrary preparation and one-copy readout; global history representation remains possible.
- **Synthesis — completed:** this understanding / reproduction task ends here. The original cyclic candidate remains rejected. Ontological becoming, equal actuality of events, and experienced passage remain unresolved. No R2 is automatically selected.
