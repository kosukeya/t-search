...
Stage 13E asks whether the Stage 12 typed operational architecture can be lifted to all 36 Stage 13 representatives and descend through the Stage 13D four-class quotient **without becoming path-word or constraint-basis dependent**.

The executable target is deliberately narrower than refoliation invariance:

1. lift O/P/R/V/Xi to all 36 canonical Stage 13 representatives;
2. confine representative, basis, path-word, raw-parameter, and compensator provenance to Xi;
3. retain exactly four quotient-level public architectures;
4. reuse the inherited Stage 10/11 future-measurement family rather than redesigning it;
5. test all 144 compensated `TX` / `XT` path pairs for public operational descent;
6. retain a two-clock, Dirac-based orbit-sensitive witness that is representative/path independent within each orbit but separates the four physical orbits;
7. reject wrong path/event/class/outcome/normalization and representative-dependent O/P/R/V/measurement controls.

## Frozen operational lift

Stage 13E carries forward
...
- **72** inherited per-continuation future-measurement views / **144** outcome probability evaluations;
- **36** weighted public views;
- **36** posterior views;
- **36** two-clock orbit-sensitive witness views with **4** distinct physical-orbit signatures;
...
`score = Q_D + 0.5 P_D + 0.25 q(T=1,X=1)`,

`p_left = 0.5 + 0.25 tanh(score)`.
