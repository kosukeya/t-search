...
Stage 8 does not establish ontically open physical reality, eternalism, collapse as becoming, phenomenal passage, general covariance, or empirical novelty.
...
- `Potentiality != absence of measurement by definition`;
- `random sampling != ontic actualization evidence`;
- `Python branch choice != physical becoming`;
- `density matrix decomposition != unique modal semantics`;
- `operational quantum equivalence != modal/ontological equivalence`;
- `record-defined orientation != ontological future openness`;
- `actualization API update != ontological becoming`.

## 3. Inherited modal distinction
...
Stage 8 preserves the selected-versus-unselected distinction on a quantum continuation carrier.

Epistemic quantum model:

`M_E^Q=(QCarrier, D, h*, q_E)`.

One complete continuation `h*` is already selected globally but hidden from the operational interface. The hidden selected continuation must not be consulted when computing the pre-discriminating operational prediction.

Ontic-extension quantum model:

`M_O^Q(D)=(QCarrier, D, QExt(D), K)`.

**No selected complete continuation datum may exist before update.** This prohibition includes hidden selectors, seed-equivalent selectors, precomputed outcomes, and latent branch variables serving the same role.

Absence of such a datum is a formal model difference, not proof that nature is ontically open.
...
## 6. QuantumContinuation and QExt(D)

A `QuantumContinuation` is an executable completion of the current quantum Actuality beyond `D`.

`QExt(D) = { [h] | h is a physically admissible quantum continuation of A_Q(D) }`

where `[h]` is a physical continuation-equivalence class. Pure renaming must not create a new class.

### Stage 8A canonical realization — completed

The canonical current anchor is `e1` and:
...
It may expose current physical/record/next-outcome information but must not expose `h*`, model type names, or privileged modal diagnostics.

Matched baseline goal:

`O_Q(G_E(D_0)) == O_Q(G_O(D_0))`
...
`q_E(h|D)` is epistemic uncertainty over a hidden already-selected continuation.

`K(h|D)` weights admissible continuations in the no-selected-future model without a stronger metaphysical commitment.

Matched numerical weights do not imply matched semantics.

An update receives explicit evidence rather than sampling a branch internally.
...
Stage 8D must use genuine Stage 7/8 interacting clock maps rather than Stage 6 event-name renaming alone.

Schematically:

`Lambda_h^q = S_{q<-p} Lambda_h^p S_{p<-q}`.

Continuation correspondence and any required `chi` must be declared before comparison.

Frozen guards:

- `same branch label across clocks != same continuation`;
- `same bare continuation matrix != corresponding continuation`;
- `P-V covariance != P=V`;
- `perspective consistency != modal equivalence`.

## 11. P/O/R/V pressure tests
...
Where applicable Stage 8 requires:

1. mere-label continuation control;
2. executable continuation inequivalence control;
3. hidden-selected-continuation swap;
4. no-selected-field audit;
5. weight mismatch;
6. superposition-is-not-V control;
7. density-matrix ambiguity control;
8. wrong/bare continuation transport;
9. wrong/misdeclared `chi`;
10. event/continuation renaming invariance;
11. invalid continuation rejection;
12. terminal-current behavior;
...
3. candidate structural interpretation;
4. untested / `not_established`.
...
Stage 8 is complete only when all applicable criteria are satisfied.

### Protocol / typing

1. central question frozen;
2. Actuality distinguished from complete continuation;
3. `QuantumContinuation` and `QExt(D)` explicit;
4. epistemic and ontic-extension Potentiality type-distinct;
5. selected-continuation presence/absence semantics explicit;
6. Potentiality not defined as superposition/Born randomness/sampling;
7. operational and privileged interfaces separated;
8. update semantics explicit;
9. genuine clock-perspective transport requirements explicit;
10. evidence taxonomy and stop/revise conditions auditable.

Stage 8.0 satisfies criteria 1–10 at the protocol level.

### Common quantum-extension substrate

11. at least two physically inequivalent continuations share one current Actuality;
12. inequivalence executable rather than label-only;
13. pure renaming does not create another physical continuation;
14. each retained continuation physically admissible;
15. invalid/current-incompatible continuation rejected;
16. terminal-current behavior explicit.

Stage 8A satisfies criteria 11–16 in the canonical continuation family.

### Typed modal models

17. epistemic model contains one selected continuation `h*`;
18. ontic-extension model contains no selected complete continuation or equivalent hidden selector;
19. both use the same quantum continuation substrate;
...
23. privileged diagnostics distinguish structures;
24. weight mismatch can change operational predictions;
25. explicit common update evidence changes Actuality consistently;
26. epistemic update retains/conditions pre-existing `h*`;
27. ontic-extension update prunes extensions without adding a hidden complete future;
28. superposition/Born randomness alone rejected as sufficient ontic-Potentiality evidence;
29. state/density representation alone does not silently decide modal semantics where applicable.

### Genuine P-V transport

30. common continuation family represented in at least two genuine interacting clock perspectives;
31. continuation correspondence and required `chi` explicit;
32. continuation maps/diagnostics transported with re-derived clock maps;
33. extension-family correspondence holds under valid transport;
34. semantic types remain distinct after transport;
35. matched operational underdetermination preserved after valid transport where expected;
36. same-bare-map/wrong-transport control fails where applicable;
37. wrong/misdeclared `chi` or continuation correspondence mismatches where applicable.

### P/O/R/V compatibility and minimality

38. same tested P/O/R construction hosts both V semantics or incompatibility explicitly established;
39. R-V relation tested without defining V by records;
40. O-V relation tested without defining V by order/history;
41. record-defined orientation does not silently determine ontic-extension semantics;
42. selected-versus-unselected V distinction ablated/reconstructed against retained P/O/R/current-state data;
43. `lost`, `reconstructible`, `inaccessible`, and `not_established` distinguished;
44. implementation details not mistaken for layer identity.

### Synthesis / next gate

45. Stage 8G classifies strengthened/reduced/broken/inconclusive;
46. unresolved implications remain `not_established` unless directly decided;
47. interpretation guards preserved;
48. next candidate gates re-ranked by discriminating power;
